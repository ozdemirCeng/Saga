...
export default function ListDetailPage() {
  ...
  const handleTogglePrivacy = async () => {
    ...
    try {
      ...
      setListe((prev) =>
        prev ? { ...prev, herkeseAcik: result.herkeseAcik } : null
      );
    } catch (err) {
          ...
          <div className="flex items-start gap-5">
            ...
            <div className="w-28 h-36 lg:w-36 lg:h-48 rounded-xl overflow-hidden bg-gradient-to-br from-[rgba(108,92,231,0.2)] to-[rgba(0,206,201,0.1)] flex-shrink-0 shadow-xl shadow-black/20">
              <div className="w-full h-full grid grid-cols-2 gap-[2px]">
                {liste.icerikler &&
                  liste.icerikler.slice(0, 4).map((icerik, i) => (
                    <div
                      key={i}
                      className="relative overflow-hidden bg-[rgba(0,0,0,0.3)]"
                    >
                      {icerik.posterUrl ? (
                        <img
                ...
                {(!liste.icerikler || liste.icerikler.length < 4) &&
                  Array(4 - (liste.icerikler?.length || 0))
                    .fill(0)
                    .map((_, i) => (
                      <div
                        key={`empty-${i}`}
            ...
            {liste.icerikler.map((icerik, index) => (
              ...
              >
                ...
                <div className="relative flex items-center gap-4 p-3">
                  ...
                  <button
                    onClick={() =>
                      navigate(`/icerik/${icerik.tur}/${icerik.icerikId}`)
                    }
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-xs text-[rgba(255,255,255,0.4)] capitalize">
                          {icerik.tur}
                        </span>
                      </div>
                      {icerik.notMetni && (
